    ...
    if(bootOverlay) {
        ...
        setTimeout(() => {
            ...
            if(app) {
                ...
                initApp();
                // OUVERTURE AUTOMATIQUE DU MODAL LÉGAL
                toggleLegal(true);
            }
        }, phase3Start);
...
// --- CHARTS & LOGIC (Inchangé, je compresse pour gagner de la place) ---
function renderThesisChart() { const ctxEl=document.getElementById('thesisChart'); if(!ctxEl)return; const ctx=ctxEl.getContext('2d'); if(window.thesisChartInstance)window.thesisChartInstance.destroy(); const gradientReal=ctx.createLinearGradient(0,0,0,400); gradientReal.addColorStop(0,'rgba(16,185,129,0.2)'); gradientReal.addColorStop(1,'rgba(16,185,129,0)'); window.thesisChartInstance=new Chart(ctx,{type:'line',data:{labels:['2020','2021','2022','2023','2024','2025 (Est.)'],datasets:[{label:'Tech Hyperscale',data:[100,135,105,145,175,160],borderColor:'#ef4444',borderDash:[5,5],borderWidth:2,tension:0.4,pointRadius:0},{label:'Real Assets',data:[100,115,140,130,145,200],borderColor:'#10b981',borderWidth:3,tension:0.4,pointRadius:4,backgroundColor:gradientReal,fill:true}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{color:'#9ca3af',usePointStyle:true,font:{family:'Inter'}}}},scales:{x:{display:true,grid:{display:false,color:'#333'},ticks:{color:'#555'}},y:{display:false}}}}); }
...
async function updatePortfolio(){ let totalVal=0; const tbody=document.getElementById('tableBody'); if(!tbody)return; tbody.innerHTML=''; for(let stock of portfolio){ const data=await fetchPriceRobust(stock.tick); if(data){stock.price=data.price;stock.prev=data.prev;} const valo=stock.price*stock.q; totalVal+=valo; const perf=((stock.price-stock.pru)/stock.pru)*100; tbody.innerHTML+=`<tr class="hover:bg-white/5 transition border-b border-white/5"><td class="px-6 py-4"><div class="text-white font-bold">${stock.name}</div><div class="text-[10px] text-gray-500">${stock.tick}</div></td><td class="px-6 py-4 text-right font-mono text-white">${stock.price.toFixed(2)} €</td><td class="px-6 py-4 text-right font-mono font-bold ${perf>=0?'text-green-400':'text-red-400'}">${(perf>0?'+':'')+perf.toFixed(2)}%</td></tr>`; } const totalEl=document.getElementById('totalVal'); if(totalEl)totalEl.innerText=Math.round(totalVal).toLocaleString()+" €"; renderSectorChart(); }
...
function initApp(){ updatePortfolio(); setInterval(updatePortfolio,60000); }
